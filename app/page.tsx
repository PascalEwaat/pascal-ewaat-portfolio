...
export default function Home() {
  ...
  // Desktop only: convert vertical wheel → horizontal scroll
  useEffect(() => {
  ...
  return (
    // md:overflow-hidden keeps desktop layout contained;
    // mobile falls through to natural page scroll
    <main className="relative flex flex-col min-h-screen bg-white md:overflow-hidden">
      <Navbar />

      {/* ── Hero tagline ── */}
      <section
        className="px-8 pb-6 flex items-start justify-between"
        style={{ paddingTop: 92 }}
      >
        ...
        {/* Right: barcode scroll indicator — desktop only */}
        <div className="hidden md:flex items-end mt-1">
          <BarcodeScrollbar
      ...
      {/* ── Project gallery ──
          Mobile:  flex-col, full-width cards, normal page scroll
          Desktop: flex-row, fixed-width cards, horizontal container scroll
      ── */}
      <section className="md:flex-1 md:flex md:flex-col md:justify-center">
        <div
          ref={scrollRef as React.RefObject<HTMLDivElement>}
          className={[
            "scroll-container",
            // Mobile: vertical stack
            "flex flex-col gap-6 px-8 pb-10",
            // Desktop: horizontal scroll row
            "md:flex-row md:gap-5 md:pt-3 md:pb-8 md:overflow-x-auto",
          ].join(" ")}
        ...
        >
          ...
          {/* Right padding on desktop */}
          <div className="hidden md:block flex-shrink-0 w-4" />
        </div>
      </section>

      {/* ── Large ghost word — desktop only ── */}
      <div className="hidden md:block px-6 pb-2 overflow-hidden pointer-events-none select-none">
      ...
      {/* ── Footer ── */}
      <footer className="flex items-center justify-between px-8 py-4 border-t border-black/10">
